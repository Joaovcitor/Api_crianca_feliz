...
const validateDataUsers = (req, res, next) => {
  ...
  if (
    ...
    validator.isEmpty(numCpf)
  ) {
    return res
      .status(400)
      .json({ errors: "Preencha todos os dados necessários!" });
  }

  if (password !== confirmepassword) {
    return res
      .status(400)
      .json({ errors: "Senhas não coincidem, tente novamente." });
  }
  ...
  if (!validator.isNumeric(numCpf) || numCpf.length < 11) {
    return res
      .status(400)
      .json({
        errors: "CPF deve conter apenas número e conter apenas 11 caracteres!",
      });
  }
