...
async function buscarIdsDosVisitadoresDoCoordenador(Model, alias, sessionId) {
  const visitadores = await Model.findAll({
    where: { coordenadorId: sessionId },
    include: [
      {
        model: Model,
        as: alias,
...
async function criaVariasNotificacoesParaOCoordenador(
  idsArrays,
  notificacao_tipo,
  descricao,
  res,
